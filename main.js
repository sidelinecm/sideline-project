// =================================================================
// ===           SIDELINE CHIANGMAI - MAIN SCRIPT (V3)           ===
// ===       Combines Best Performance with Full Features        ===
// =================================================================

// --- PART 1: IMMEDIATE UI SETUP (Lightweight & Fast) ---
// This code runs as soon as the HTML is ready (DOMContentLoaded).
// It sets up the most critical UI elements without waiting for heavy libraries.
document.addEventListener('DOMContentLoaded', () => {
    // 1. Initialize non-dynamic UI elements immediately for a fast perceived load time.
    initializeStaticUI();

    let heavyScriptsInitialized = false;

    // 2. This function will be called ONLY when we're ready to load the heavy stuff.
    const initializeHeavyScripts = () => {
        if (heavyScriptsInitialized) return; // Prevent running more than once
        heavyScriptsInitialized = true;
        
        console.log("User interaction detected or timeout reached. Loading full application (Supabase, GSAP)...");
        runFullApp(); 
    };

    // 3. Set up triggers to load the main application logic.
    // We load it as soon as the user shows any sign of interaction.
    window.addEventListener('scroll', initializeHeavyScripts, { once: true, passive: true });
    window.addEventListener('mousemove', initializeHeavyScripts, { once: true, passive: true });
    window.addEventListener('touchstart', initializeHeavyScripts, { once: true, passive: true });
    
    // 4. Fallback timer: If the user does nothing for 3.5 seconds, load it anyway.
    setTimeout(initializeHeavyScripts, 3500);
});


// --- PART 2: STATIC UI INITIALIZATION ---
// This function only contains code that has NO external dependencies.
// It's designed to be extremely fast.
function initializeStaticUI() {
    console.log("Initializing Static UI...");
    ...
    // --- Fade in the body to prevent flash of unstyled content ---
    document.body.style.transition = 'opacity 0.5s ease-in-out';
    document.body.classList.remove('opacity-0');
}


// --- PART 3: THE FULL APPLICATION (Heavy Lifting) ---
// This function contains EVERYTHING that requires Supabase and GSAP.
// It will only be called after user interaction or a timeout.
async function runFullApp() {
    
    // --- Step 1: Dynamically import the heavy libraries ---
    ...
    // --- Step 2: Define all constants, variables, and DOM elements ---
    ...
    // --- Step 3: Define all application functions ---
    // (These are the well-written functions from your latest version)
    
    // (The content of all functions from your last main.js goes here)
    // For brevity, I'll list the function names. The content is identical
    // to your last submission.
    ...
    function renderProfiles(filteredProfiles, isSearching) {
        ...
        if (dom.featuredSection) {
            ...
            if (currentPage === 'home' && !isSearching && featuredProfilesList.length > 0) {
                ...
                dom.featuredContainer.append(...featuredProfilesList.map(p => createProfileCard(p, gsap)));
        ...
        if (currentPage === 'profiles' || (currentPage === 'home' && isSearching)) {
            ...
            if (isSearching && currentPage === 'home') {
                 const searchResultWrapper = createSearchResultSection(filteredProfiles, gsap);
                 ...
                 gridContainer.append(...filteredProfiles.map(p => createProfileCard(p, gsap)));
            ...
            [...provincesMap.keys()].forEach(provinceKey => {
                ...
                if (provinceProfiles.length > 0) {
                    ...
                    const provinceSectionEl = createProvinceSection(provinceKey, provinceName, provinceProfiles, gsap);
    ...
    function createProvinceSection(key, name, provinceProfiles, gsap) {
        ...
        grid.append(...profilesToDisplay.map(p => createProfileCard(p, gsap)));
    ...
    function createSearchResultSection(profiles, gsap) {
        ...
        grid.append(...profiles.map(p => createProfileCard(p, gsap)));
        return wrapper;
    }
    ...
    function initAgeVerification() {
        ...
        overlay.classList.add('flex'); // Add flex to center content
        ...
        if (confirmBtn) { confirmBtn.addEventListener('click', closeAction); }
        if (cancelBtn) { cancelBtn.addEventListener('click', () => { window.location.href = 'https://www.google.com'; }); }
    }
    ...
    function initLightbox() {
        ...
        document.body.addEventListener('keydown', (event) => {
            if (event.key === 'Enter' && event.target.closest('.profile-card-new')) {
                event.preventDefault(); openAction(event.target.closest('.profile-card-new'));
    ...
    function populateLightbox(profileData) {
        // This function is the improved one from your latest version.
    ...
    // --- Step 4: The final initialization call ---
    // This is the main entry point for the heavy application logic.
    async function main() {
        initMobileMenu();
        initAgeVerification();
