...
(function () {
  ...
  function processProfileObject(raw) {
    ...
    const availStatus = (raw.availability || raw.status || "รับงาน").trim();
    const isBusy = ["ติดจอง", "ไม่ว่าง", "พัก", "หยุด", "off", "busy"].some(keyword => availStatus.toLowerCase().includes(keyword));
    const isInquire = availStatus.includes("สอบถาม") || availStatus.includes("จอง") || availStatus.includes("คิว");
    const isAvailable = !isBusy && !isInquire;
  ...
  function renderSearchSuggestions(query) {
    ...
    if (!q) {
      ...
      zones.forEach(z => {
        html += `<span data-action="suggestion" data-slug="${escapeHTML(z)}" data-is-profile="false" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #D4D4D8; font-size: 11px; font-weight: 700; padding: 4px 10px; border-radius: 100px; cursor: pointer;">📍 ${escapeHTML(z)}</span>`;
      });
    ...
    if (matchedProvinces.length > 0) {
      ...
      matchedProvinces.forEach(([key, name]) => {
        html += `
          <div onclick="window.location.href='/location/${escapeHTML(key)}'" style="padding: 10px 14px; cursor: pointer; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.05); color: #FFF; font-size: 13px; font-weight: 700; transition: background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='transparent'">
            <span>📍 ไซด์ไลน์${escapeHTML(name)}</span>
      ...
      profileMatches.forEach(item => {
        ...
        html += `
          <div class="suggestion-item" data-action="suggestion" data-slug="${encodeURIComponent(item.slug || item.id)}" data-is-profile="true" style="display: flex; align-items: center; gap: 10px; padding: 10px 14px; cursor: pointer; border-bottom: 1px solid rgba(255,255,255,0.03); transition: background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='transparent'">
              <img src="${escapeHTML(thumbImg)}" style="width: 36px; height: 36px; border-radius: 50%; object-fit: cover; border: 1px solid rgba(255,255,255,0.1);">
              <div style="flex: 1; min-width: 0; text-align: left;">
                  <div style="font-size: 13px; font-weight: 800; color: #FFFFFF;">${escapeHTML(item.displayName || item.name)}</div>
                  <div style="font-size: 11px; color: #A1A1AA;">📍 ${escapeHTML(item.location || item.provinceNameThai)}</div>
              </div>
              <span style="color: #00E676; font-weight: 800; font-size: 12px;">${escapeHTML(item.displayPrice)}</span>
          </div>
        `;
      });
    }
  ...
  function createProfileCardElement(profile, index = 20) {
    ...
    const availRaw = (profile.availability || profile.status || "").trim();
    ...
    } else if (["ไม่ว่าง", "ติดจอง", "พัก", "หยุด", "off", "busy"].some(k => availRaw.toLowerCase().includes(k))) {
    ...
    card.innerHTML = `
      <img src="${escapeHTML(imageSrc)}" 
           alt="${escapeHTML(seoAltText)}"
           title="${escapeHTML(seoAltText)}"
           width="300"
           height="400"
      ...
      <a href="/sideline/${encodedSlug}" class="card-link" style="position: absolute; inset: 0; z-index: 25;" aria-label="ดูโปรไฟล์${escapeHTML(nameClean)}"></a>
      ...
      <div style="position: absolute; bottom: 0; left: 0; right: 0; padding: 10px 11px 12px 11px; z-index: 20; pointer-events: none; text-align: left; display: flex; flex-direction: column; gap: 1px;">
          <h3 style="font-size: 14px; font-weight: 800; color: white; margin: 0; line-height: 1.2; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; text-shadow: 0 2px 4px rgba(0,0,0,0.95);">
            ${escapeHTML(nameClean)}${escapeHTML(ageDisplay)}
          </h3>
          
          ${(profile.slogan || profile.quote) ? `<p style="font-size: 11px; color: #f0a0cd; font-weight: 600; margin: 0; line-height: 1.2; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; text-shadow: 0 1px 2px rgba(0,0,0,0.95);">${escapeHTML(profile.slogan || profile.quote)}</p>` : ''}
          ...
          <div style="display: flex; align-items: center; justify-content: space-between; font-size: 10px; color: #eee7f2; border-top: 1px solid rgba(255,255,255,0.08); padding-top: 3px; margin-top: 2px;">
              <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 60%; font-weight: 600; text-shadow: 0 1px 2px rgba(0,0,0,0.95);">
                  <i class="fas fa-map-marker-alt" style="color: #C084FC; margin-right: 2px;"></i> ${escapeHTML(profile.location || currentProvName)}
              </span>
              <span style="color: #00E676; font-weight: 900; font-size: 12px; text-shadow: 0 1.5px 3px rgba(0,0,0,0.95);">
                  ${escapeHTML(profile.displayPrice)}
              </span>
          </div>
      </div>
    `;
  ...
  function createProvinceSectionElement(provinceKey, provinceName, profiles) {
    ...
    wrapper.innerHTML = `
      <div style="padding: 8px 4px 12px 4px;">
          <a href="/location/${escapeHTML(provinceKey)}" class="group" style="text-decoration: none; display: inline-block;">
              <h2 class="province-section-header" style="display: flex; align-items: center; flex-wrap: wrap; gap: 8px; font-size: 18px; font-weight: 800; color: white; margin: 0;">
                  📍 น้องๆ ในจังหวัด <span style="color: #C084FC;">${escapeHTML(provinceName)}</span>
                  <span class="live-count-chip">
                    <span class="pulse-dot-el"></span>
  ...
  function openLightboxForProfile(profile) {
    ...
    if (badgeEl) {
      badgeEl.innerHTML = `
        <span style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.15); padding: 4px 12px; border-radius: 100px; display: inline-flex; align-items: center; gap: 6px; backdrop-filter: blur(8px);">
            <span style="width: 6px; height: 6px; border-radius: 50%; background: ${statusDotColor}; box-shadow: 0 0 8px ${statusDotColor};"></span>
      ...
      detailsEl.innerHTML = `
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-bottom: 10px;">
            <div class="bento-stat-tile">
                ...
                <div style="font-weight: 900; font-size: 13px; color: #FFFFFF; margin-top: 2px;">${escapeHTML(ageDisplay)}</div>
            </div>
            <div class="bento-stat-tile">
                <div style="font-size: 10.5px; color: #A1A1AA; font-weight: 700;">สัดส่วน</div>
                <div style="font-weight: 900; font-size: 13px; color: #FFFFFF; margin-top: 2px;">${escapeHTML(statsText)}</div>
            </div>
            <div class="bento-stat-tile">
                <div style="font-size: 10.5px; color: #A1A1AA; font-weight: 700;">ส่วนสูง</div>
                <div style="font-weight: 900; font-size: 13px; color: #FFFFFF; margin-top: 2px;">${escapeHTML(heightText)}</div>
            </div>
        </div>
        ...
        <div style="background: linear-gradient(135deg, rgba(255, 255, 255, 0.04) 0%, rgba(255, 255, 255, 0.01) 100%); padding: 12px 14px; border-radius: 16px; border: 1px solid rgba(255, 255, 255, 0.06); display: flex; flex-direction: column; gap: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.3);">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <span style="color: #00E676; font-weight: 900; font-size: 14px; text-shadow: 0 0 10px rgba(0, 230, 118, 0.4);">${escapeHTML(profile.displayPrice)}</span>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px dashed rgba(255,255,255,0.06); padding-top: 6px;">
                ...
                <span style="color: white; font-weight: 800; font-size: 12px;">${escapeHTML(profile.location || profile.provinceNameThai || "เชียงใหม่")}</span>
            </div>
        </div>
      `;
    }
    ...
    if (descContent) {
      ...
      const finalDesc = profile.description || defaultDesc;
    ...
    if (detailsContainer) {
      ...
      stickyBtnWrapper.innerHTML = `
        <a href="${lineUrl}" target="_blank" rel="noopener nofollow" class="lightbox-line-cta">
            ...
            <span>แอดไลน์จองคิว ${escapeHTML(nameClean)}</span>
        </a>
      `;
  ...
  function replaceDomPlaceholders(provinceName, profileCount, provinceSlug) {
    try {
      ...
      const currentZones = (data.zones && data.zones.length > 0)
        ...
        : (isNational ? "เชียงใหม่, ขอนแก่น, เชียงราย, ลำปาง, อุดรธานี, ภูเก็ต" : "ตัวเมือง และพื้นที่ใกล้เคียง");
        ...
        faqContainer.innerHTML = data.faqs.map(item => `
          <div class="faq-item-card">
            <div class="faq-q-row">
               ...
               <div class="faq-question-text">${escapeHTML(item.q)}</div>
            </div>
            <div class="faq-answer-text">${escapeHTML(item.a)}</div>
          </div>
        `).join("");
      }
          ...
          <div class="review-card-item">
             <div class="review-card-header">
                <div class="review-user-info">
                   <div class="review-avatar-circle">${escapeHTML(r.initial || 'V')}</div>
                   <div>
                      <div class="review-username">${escapeHTML(r.name)}</div>
                      <div class="review-user-loc">${escapeHTML(r.loc)}</div>
                   </div>
                </div>
  ...
  document.addEventListener("DOMContentLoaded", async function () {
    ...
    document.querySelectorAll(".region-tab").forEach(tab => {
      tab.addEventListener("click", () => {
        ...
        else if (region === "ภาคอีสาน") window.location.href = "/location/khonkaen";
      });
    });
