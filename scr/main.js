...
// เวอร์ชัน: 3.0 (Stability & Performance Fix)
// คำอธิบาย: แก้ไขข้อผิดพลาดร้ายแรง "Cannot read properties of null" ที่เกิดจากการพยายามเข้าถึง
// element ของฟอร์มค้นหาในหน้าที่ไม่มีฟอร์มอยู่ ทำให้เว็บจอขาว
// พร้อมทั้งรวมการปรับปรุง Performance ทั้งหมดไว้ในไฟล์เดียว
...
// --- รวมศูนย์กลางการจัดการ DOM Elements ที่มีอยู่ทุกหน้า ---
const dom = {
    body: document.body,
...
async function initializeApp() {
    ...
    if (['home', 'profiles'].includes(currentPage)) {
        ...
        if (success) {
            initSearchAndFilters(); // [FIX] ฟังก์ชันนี้ถูกแก้ไขให้ปลอดภัยแล้ว
            initLightbox();
...
function loadDelayedScripts() {
    ...
    // Google Analytics ถูกลบออกตามแผนปรับ Performance
}
...
// (ฟังก์ชันในส่วนนี้ทั้งหมดเหมือนเดิมทุกประการ)

function initCriticalUI() { initThemeToggle(); initMobileMenu(); initHeaderScrollEffect(); updateActiveNavLinks(); }
...
function showLoadingState() { if (dom.fetchErrorMessage) dom.fetchErrorMessage.classList.add('hidden'); if (dom.noResultsMessage) dom.noResultsMessage.classList.add('hidden'); if (dom.loadingPlaceholder) { const grid = dom.loadingPlaceholder.querySelector('.grid'); if (grid && grid.innerHTML === '') { grid.innerHTML = Array(SKELETON_CARD_COUNT).fill('<div class="skeleton-card"></div>').join(''); } dom.loadingPlaceholder.style.display = 'block'; } }
function hideLoadingState() { if (dom.loadingPlaceholder) dom.loadingPlaceholder.style.display = 'none'; }
function showErrorState(customMessage = "ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์ได้ในขณะนี้") { hideLoadingState(); if (dom.profilesDisplayArea) dom.profilesDisplayArea.innerHTML = ''; if (dom.featuredSection) dom.featuredSection.classList.add('hidden'); if (dom.fetchErrorMessage) { const errorDesc = dom.fetchErrorMessage.querySelector('.error-description'); if (errorDesc) errorDesc.textContent = customMessage; dom.fetchErrorMessage.classList.remove('hidden'); } }
function renderProfiles(filteredProfiles, isSearching) {
    ...
    if (filteredProfiles.length === 0 && (isSearching || currentPage === 'profiles')) {
        ...
        const displayLogic = {
            'profiles': () => { const gridContainer = document.createElement('div'); gridContainer.className = 'grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6'; gridContainer.append(...filteredProfiles.map((profile, index) => createProfileCard(profile, index))); dom.profilesDisplayArea.appendChild(gridContainer); },
            'home': () => {
                if (isSearching) { const searchResultWrapper = document.createElement('div'); searchResultWrapper.className = 'grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6'; searchResultWrapper.append(...filteredProfiles.map((profile, index) => createProfileCard(profile, index))); dom.profilesDisplayArea.appendChild(searchResultWrapper); } else {
        ...
        if (displayLogic[currentPage]) { displayLogic[currentPage](); }
    }
}
function createProfileCard(profile, index, isEager = false) { const card = document.createElement('div'); card.className = 'profile-card-new group cursor-pointer'; card.dataset.profileId = profile.id; card.setAttribute('aria-label', `ดูโปรไฟล์ของ ${profile.name}`); card.setAttribute('role', 'button'); card.tabIndex = 0; card.dataset.animateOnScroll = ''; const mainImage = profile.images[0]; const isAboveTheFold = index < ABOVE_THE_FOLD_COUNT; const img = document.createElement('img'); img.src = mainImage.medium; img.srcset = `${mainImage.small} 400w, ${mainImage.medium} 600w`; img.sizes = "(max-width: 640px) 50vw, (max-width: 768px) 33vw, 25vw"; img.alt = profile.altText; img.className = "card-image"; img.decoding = "async"; img.width = 300; img.height = 400; if (isAboveTheFold || isEager) { img.loading = 'eager'; img.setAttribute('fetchpriority', 'high'); } else { img.loading = 'lazy'; } img.onerror = () => { img.onerror = null; img.src = '/images/placeholder-profile.webp'; img.srcset = ''; }; let availabilityText = profile.availability || "สอบถามคิว"; let availabilityClass = 'bg-yellow-200 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300'; if (availabilityText.includes('ว่าง') || availabilityText.includes('รับงาน')) { availabilityClass = 'bg-green-200 text-green-800 dark:bg-green-900/50 dark:text-green-300'; } else if (availabilityText.includes('ไม่ว่าง') || availabilityText.includes('พัก')) { availabilityClass = 'bg-red-200 text-red-800 dark:bg-red-900/50 dark:text-red-300'; } const starIcon = `<svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10.868 2.884c.321-.662 1.134-.662 1.456 0l2.034 4.192a.75.75 0 00.564.41l4.625.672c.728.106 1.018.995.494 1.503l-3.348 3.263a.75.75 0 00-.215.664l.79 4.607c.124.724-.636 1.285-1.288.941l-4.135-2.174a.75.75 0 00-.696 0l-4.135 2.174c-.652.344-1.412-.217-1.288-.94l.79-4.607a.75.75 0 00-.215-.665L1.15 9.66c-.524-.508-.234-1.397.494-1.503l4.625-.672a.75.75 0 00.564-.41L9.132 2.884z" clip-rule="evenodd" /></svg>`; const locationIcon = `<svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M9.69 18.933l.003.001C9.89 19.02 10 19 10 19s.11.02.308-.066l.002-.001.006-.003.018-.008a5.741 5.741 0 00.281-.14c.186-.1.4-.223.654-.369.623-.359 1.445-.835 2.13-1.36.712-.549 1.282-1.148 1.655-1.743.372-.596.59-1.28.59-2.002v-1.996a4.504 4.504 0 00-1.272-3.116A4.47 4.47 0 0013.5 4.513V4.5C13.5 3.12 12.38 2 11 2H9c-1.38 0-2.5 1.12-2.5 2.5v.013a4.47 4.47 0 00-1.728 1.388A4.504 4.504 0 003 9.504v1.996c0 .722.218 1.406.59 2.002.373.595.943 1.194 1.655 1.743.685.525 1.507 1.001 2.13 1.36.254.147.468.27.654-.369a5.745 5.745 0 00.28.14l.019.008.006.003zM10 11.25a2.25 2.25 0 100-4.5 2.25 2.25 0 000 4.5z" clip-rule="evenodd" /></svg>`; card.innerHTML = ` <div class="absolute top-2 right-2 flex flex-col items-end gap-1.5 z-10"> <span class="${availabilityClass} text-xs font-semibold px-2.5 py-1 rounded-full shadow-lg">${availabilityText}</span> ${profile.isfeatured ? `<span class="bg-yellow-400 text-black text-xs font-semibold px-2.5 py-1 rounded-full flex items-center gap-1 shadow-lg">${starIcon}แนะนำ</span>` : ''} </div> <div class="card-overlay"> <div class="card-info"> <h3 class="text-xl lg:text-2xl font-bold truncate">${profile.name}</h3> <p class="text-sm flex items-center gap-1.5">${locationIcon} ${provincesMap.get(profile.provinceKey) || 'ไม่ระบุ'}</p> </div> </div>`; card.prepend(img); return card; }

// =================================================================
// SECTION 5: ตัวจัดการเหตุการณ์และฟังก์ชันเสริม (EVENT HANDLERS & HELPERS)
// =================================================================

async function handleRetry() { if (dom.fetchErrorMessage) dom.fetchErrorMessage.classList.add('hidden'); showLoadingState(); const success = await fetchData(); if (success) { applyFilters(); } else { showErrorState(); } }

/**
 * [FIX] แก้ไขฟังก์ชันให้ปลอดภัย โดยตรวจสอบก่อนว่าฟอร์มค้นหามีอยู่จริงหรือไม่
 */
function initSearchAndFilters() {
    const searchForm = document.getElementById('search-form');
    // ถ้าไม่เจอ search form ในหน้านี้ ให้จบการทำงานทันที
    if (!searchForm) {
        if(allProfiles.length > 0) applyFilters();
        return;
    }
    
    // หา element ต่างๆ ของฟอร์ม (จะทำงานก็ต่อเมื่อเจอ searchForm แล้วเท่านั้น)
...
/**
 * [FIX] แก้ไขฟังก์ชันให้ปลอดภัย โดยตรวจสอบก่อนว่า element มีอยู่หรือไม่
 */
...
async function loadAnimationScripts() { if (gsap) return; try { const [gsapModule, scrollTriggerModule] = await Promise.all([ import("https://cdn.jsdelivr.net/npm/gsap@3.12.5/+esm"), import("https://cdn.jsdelivr.net/npm/gsap@3.12.5/ScrollTrigger/+esm") ]); gsap = gsapModule.gsap; ScrollTrigger = scrollTriggerModule.ScrollTrigger; gsap.registerPlugin(ScrollTrigger); } catch (error) { console.error('Error loading animation scripts (GSAP). Animations will be disabled.', error); } }
async function initScrollAnimations() { await loadAnimationScripts(); if (!gsap) return; ScrollTrigger.getAll().forEach(trigger => trigger.kill()); ScrollTrigger.refresh(); const animatedElements = document.querySelectorAll('[data-animate-on-scroll]'); animatedElements.forEach(el => { gsap.from(el, { scrollTrigger: { trigger: el, start: "top 95%", toggleActions: "play none none none", }, opacity: 0, y: 30, duration: 0.6, ease: "power2.out", }); }); const heroH1 = document.querySelector('#hero-h1'); if (heroH1 && !heroH1.classList.contains('gsap-animated')) { heroH1.classList.add('gsap-animated'); const heroElements = [heroH1, document.querySelector('#hero-p'), document.querySelector('#hero-form')].filter(Boolean); gsap.from(heroElements, { y: 20, opacity: 0, duration: 0.6, stagger: 0.15, ease: 'power2.out', delay: 0.2 }); } }
