...
async function handleProfilePage(supabase, slug) {
    ...
    const isVerified = p.isfeatured === true;
        ...
        <div class="content">
            ...
            <div class="gallery-grid">
                ${galleryImages.map((img, i) => {
                    ...
                    const extraKwd = seoKeywords[i % seoKeywords.length];
...
<head>
    ...
    <title>${isBot ? title.replace(/<span><\/span>/g, '') : title}</title>
    <meta name="description" content="${desc.replace(/<span><\/span>/g, '')}">
            <meta name="rating" content="adult">
    <link rel="canonical" href="${canonicalUrl}">
    <meta property="og:title" content="${title}">
    <meta property="og:image" content="${CONFIG.OG_PREVIEW}">
    <meta property="og:url" content="${canonicalUrl}">
    <meta property="og:type" content="website">
    <script type="application/ld+json">${JSON.stringify(schemaData)}</script>
    <style>
        :root{--p:#ec4899;--bg:#0f172a;--c:#1e293b;--t:#f1f5f9;--s:#10b981}
        body{font-family:'Prompt',sans-serif;background:var(--bg);color:var(--t);margin:0;line-height:1.6}
        .container{max-width:1000px;margin:0 auto;padding:20px}
        header{text-align:center;padding:45px 20px;background:linear-gradient(180deg, #1e293b 0%, #0f172a 100%);border-bottom:3px solid var(--p)}
        ...
        /* Grid System */
        .grid{display:grid;grid-template-columns:repeat(auto-fill, minmax(180px, 1fr));gap:20px;margin:35px 0}
        .card{background:var(--c);border-radius:15px;overflow:hidden;text-decoration:none;color:inherit;border:1px solid #334155;transition:0.3s;position:relative}
        .card:hover{transform:translateY(-5px);border-color:var(--p)}
        .card-img-box{position:relative;padding-top:135%;background:#1e293b}
        .card-img-box img{position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover}
        
        /* Badge & Featured */
        .featured-card { border: 2px solid var(--p) !important; box-shadow: 0 0 20px rgba(236,72,153,0.2); }
        .f-badge { position:absolute; top:10px; left:10px; background:var(--p); color:#fff; padding:3px 10px; border-radius:6px; font-size:11px; font-weight:bold; z-index:5; box-shadow:0 2px 5px rgba(0,0,0,0.3); }
        .v-check { position:absolute; bottom:10px; right:10px; background:var(--s); color:#fff; width:22px; height:22px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:12px; z-index:5; border:2px solid #fff; }
        
        /* Content Sections */
        .box{background:var(--c);padding:30px;border-radius:20px;margin:30px 0;border:1px solid #334155}
        .trust-grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:15px;margin:20px 0}
        .trust-item{text-align:center;font-size:13px;background:rgba(255,255,255,0.03);padding:15px;border-radius:12px}
        .trust-item b{color:var(--p);display:block;font-size:16px}
        
        h1,h2,h3{color:var(--p);margin-top:0}
        .faq-item{margin-bottom:20px;border-bottom:1px solid #334155;padding-bottom:15px}
        .faq-q{font-weight:bold;color:var(--p);display:block;margin-bottom:5px}
        .faq-a{font-size:14px;color:#94a3b8}
...
<body>
    <header>
        ...
        <h1 style="color:#fff;font-size:26px;margin-top:20px;letter-spacing:1px">${mask(`รวมไซด์ไลน์${provinceName} รับงานเอง`)}</h1>
        <p style="color:#94a3b8;font-size:14px">พบกับน้องๆ เพื่อนเที่ยว VIP พิกัด${provinceName} ตัวจริงตรงปก 100%</p>
    </header>

    <main class="container">
        <div style="text-align:center;margin-bottom:30px">
            <p style="font-size:13px;color:#64748b;margin-bottom:10px uppercase">📍 พิกัดยอดนิยมใน${provinceName}</p>
            ${localZones.map(z => `<a href="#" class="z-tag">${z}</a>`).join('')}
        </div>

        <div class="grid">${profileGridHTML}</div>

        <div class="box">
            <h2>🛡️ ทำไมต้องเลือกหาเพื่อนเที่ยว${provinceName}กับเรา?</h2>
            <p>เราคือแพลตฟอร์มอันดับ 1 ในการรวบรวมโปรไฟล์น้องๆ <strong>${mask(`ไซด์ไลน์${provinceName}`)}</strong> ที่ทำงานอิสระ ไม่ผ่านเอเย่นต์ มั่นใจได้ว่าข้อมูลถูกต้องและปลอดภัยที่สุด</p>
            
            <div class="trust-grid">
                <div class="trust-item"><b>No Deposit</b>ไม่ต้องโอนมัดจำก่อน</div>
                <div class="trust-item"><b>Verified</b>รูปตรงปก ตรวจสอบได้</div>
                <div class="trust-item"><b>Direct Pay</b>จ่ายเงินให้น้องหน้างาน</div>
            </div>
        </div>
        ...
        <div class="box">
            ...
            <div class="faq-item">
                <span class="faq-q">นัดน้องใน${provinceName} ต้องรอนานไหม?</span>
                <span class="faq-a">ส่วนใหญ่น้องๆ จะสแตนบายในย่าน ${localZones.slice(0,3).join(', ')} ใช้เวลาเดินทางเพียง 30-45 นาทีถึงพิกัดพี่ๆ ค่ะ</span>
            </div>
            <div class="faq-item">
                <span class="faq-q">จะมั่นใจได้อย่างไรว่าไม่โดนหลอก?</span>
                <span class="faq-a">เว็บไซต์เรามีนโยบายเด็ดขาดคือ <strong>"ห้ามโอนเงินก่อนทุกกรณี"</strong> พี่ๆ จะจ่ายค่าขนมเมื่อเจอน้องตัวจริงแล้วเท่านั้น มั่นใจปลอดภัย 100% ค่ะ</span>
            </div>
            <div class="faq-item">
                <span class="faq-q">รับงานนอกสถานที่หรือในโรงแรมไหม?</span>
                <span class="faq-a">น้องๆ ส่วนใหญ่รับทั้งในพิกัดส่วนตัวและโรงแรมชั้นนำทั่ว${provinceName} ค่ะ สามารถสอบถามรายละเอียดการเดินทางได้โดยตรงทางไลน์</span>
            </div>
        </div>

        <div style="text-align:center;margin:40px 0;padding:20px;border-top:1px solid #334155">
            <p style="color:#64748b;font-size:14px;margin-bottom:15px">ค้นหาเพื่อนเที่ยวในจังหวัดอื่นๆ</p>
            ${otherLocs.map(l => `<a href="/location/${l.s}" class="z-tag">ไซด์ไลน์${l.n}</a>`).join('')}
        </div>
    </main>

    <footer style="text-align:center;padding:30px;color:#475569;font-size:12px">
        <p>© ${new Date().getFullYear()} ${CONFIG.BRAND_NAME} - Premium VIP Directory</p>
        <p>อัปเดตข้อมูลล่าสุดเมื่อ: ${formatDate()}</p>
    </footer>
</body>
</html>`, { 
    headers: { 
