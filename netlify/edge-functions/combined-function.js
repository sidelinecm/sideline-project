...
const CONFIG = {
    ...
    SITE_NAME: 'Sideline Chiang Mai (ไซด์ไลน์เชียงใหม่)', // ✨ ENHANCED: For og:site_name and Brand schema
    SOCIAL_PROFILES: [
...
// ✨ NEW: Helper for Wikidata IDs for powerful local SEO signals
const getProvinceWikidata = (provinceSlug) => {
    const provinceData = {
        'chiangmai': 'Q42430',
        'bangkok': 'Q1861',
        'chonburi': 'Q207606',
    ...
    return provinceData[provinceSlug.toLowerCase()] || null;
};
...
const generatePageData = (provinceName, zones) => {
    return {
        faq: [
            ...
            { q: `เรทราคาเริ่มต้นเท่าไหร่?`, a: `ค่าขนมเริ่มต้นที่ 1,500 - 2,000 บาท ขึ้นอยู่กับโปรไฟล์และประเภทงาน สามารถดูราคาชัดเจนได้ที่หน้าโปรไฟล์น้องๆ แต่ละคนครับ` },
            ...
            { q: `ข้อมูลและรูปภาพตรงปกไหม?`, a: `ทีมงาน ${CONFIG.BRAND_NAME} มีการตรวจสอบตัวตนและคัดกรองน้องๆ อย่างเข้มงวด รับประกันว่ารูปตรงปกและให้บริการด้วยความสุภาพครับ` }
        ]
    };
};
...
export default async (request, context) => {
    try {
        ...
        const provinceWikidataId = getProvinceWikidata(provinceSlug);
        ...
        if (p.imagePath) {
            imageUrl = p.imagePath.startsWith('http') ? p.imagePath : `${CONFIG.SUPABASE_URL}/storage/v1/object/public/profile-images/${p.imagePath}?width=800&quality=85&format=webp`;
        }
        
        const lineUrl = lineId && !lineId.startsWith('http') ? `https://line.me/ti/p/${lineId}` : (lineId || '#');
            ...
            "@graph": [
                ...
                },
                {
                    ...
                    "review": {
                        ...
                        "reviewBody": `${displayName} งานดีมากครับ ตรงปกไม่จกตา บริการเป็นกันเองสุดๆ`,
                    ...
                    ...(provinceWikidataId ? { 
                        "areaServed": {
                            ...
                            "name": provinceName,
                            "sameAs": `https://www.wikidata.org/wiki/${provinceWikidataId}`
                        }
                    } : {})
                },
                {
    ...
    <main class="container">
        <header>
            ...
            <div class="hero">
                ...
                <img src="${imageUrl}" alt="โปรไฟล์ของ ${displayName}, ไซด์ไลน์${provinceName}" width="800" height="1000" fetchpriority="high" decoding="async">
        ...
        <article class="content">
            <section aria-labelledby="profile-heading">
                ...
                <h1 id="profile-heading">${pageTitle}</h1>
                <div class="location">📍 พิกัด: ${location}</div>

                <div class="info-grid">
                    <div class="info-item"><span class="info-label">ค่าขนม</span><span class="info-val price">฿${displayPrice}</span></div>
                    <div class="info-item"><span class="info-label">อายุ</span><span class="info-val">${age} ปี</span></div>
                    <div class="info-item"><span class="info-label">สัดส่วน</span><span class="info-val">${proportions}</span></div>
                    <div class="info-item"><span class="info-label">สูง/หนัก</span><span class="info-val">${height} / ${weight}</span></div>
                </div>
            ...
            <aside class="related" aria-labelledby="related-heading">
                <h2 id="related-heading">🔥 น้องๆ แนะนำใน${provinceName}</h2>
                <div class="related-grid">
                    ${related.map(r => `
                        <a href="${CONFIG.DOMAIN}/sideline/${r.slug}" class="related-card">
                            <!-- ✨ PERFORMANCE OPTIMIZED RELATED IMAGES -->
                            <img src="${CONFIG.SUPABASE_URL}/storage/v1/object/public/profile-images/${r.imagePath}?width=250&quality=80" alt="รูปโปรไฟล์น้อง${r.name}" loading="lazy" decoding="async" width="250" height="300">
        ...
        <footer class="main-footer">
            <p>&copy; ${new Date().getFullYear()} ${CONFIG.SITE_NAME}<br>
            ให้บริการใน${provinceName}และจังหวัดอื่นๆ ทั่วประเทศ</p>
        </footer>
    </main>
</body>
</html>`;
