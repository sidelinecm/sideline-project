...
export default async (request, context) => {
    ...
    try {
        ...
        // --- 🛠️ แก้ไขจุดบอด: ตรวจสอบข้อมูลก่อนแสดงผล (Logic ขั้นสูง) ---
        
        // จัดการอายุ (ห้ามมีคำว่า null)
        const ageText = (profile.age && profile.age !== 'null') ? `${profile.age} ปี` : '20+ ปี';
        
        // จัดการสัดส่วน (ถ้าไม่มี ไม่ต้องแสดงเลย)
        const statsHtml = (profile.stats && profile.stats !== '-' && profile.stats !== 'null')
            ? `<div class="meta-item"><span class="meta-label">สัดส่วน</span><span class="meta-val">${profile.stats}</span></div>`
            : '';

        // จัดการรูปภาพ (Absolute URL)
        ...
        // 4. SEO & Fake Reviews Logic (ดึงดูดการคลิก)
        ...
        const metaDesc = `รีวิว ${rating}⭐ (${reviewCount} คน): น้อง${name} ไซด์ไลน์${province} โซน${location} อายุ ${ageText} รูปจริงตรงปก ไม่เร่งงาน ปลอดภัย จ่ายหน้างาน 100%`;
