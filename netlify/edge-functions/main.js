...
(function () {
  ...
  // 🟢 1. FIX HOT SWIPER CARDS (Font Size >= 12px & Correct Pointer Events)
  ...
  // 🟢 2. FIX PROFILE CARD ELEMENT (Font Sizes >= 12px for Lighthouse)
  ...
  // 🟢 3. FIX LIGHTBOX PROFILE NAME (Hero Neon Pink Gradient Style)
  function openLightboxForProfile(profile) {
    ...
    // 🌟 นีออนชมพู สไตล์เดียวกับ Hero Brand Neon Text (FirstModelHub🌟)
  ...
  function updateDynamicProvinceContent(provKey = "chiangmai", provName = "เชียงใหม่", count = 50) {
    ...
    if (heroSub) {
// ดึงโซนของจังหวัดนั้นๆ ถ้าไม่มี ให้แสดง "ตัวเมือง, บริเวณใกล้เคียง" แทน ไม่แสดงชื่อจังหวัดอื่น
    const currentZones = (data && data.zones && data.zones.length > 1) 
      ? data.zones.slice(1, 6) 
      : ["ตัวเมือง", "บริเวณใกล้เคียง"];ง"];
      heroSub.innerHTML = `
  ...
  document.addEventListener("DOMContentLoaded", async function () {
    ...
    // 🟢 จัดการ Event Click ทั่วไป (รวมปุ่ม Like, Search, และการ์ดน้องๆ ทุกส่วน)
    document.body.addEventListener("click", e => {
      ...
      // 🟢 FIX 1: ตรวจจับการกดลิงก์บนการ์ด (.profile-card-new และ .vip-card-item) ให้เปิด Lightbox ได้ 100%
