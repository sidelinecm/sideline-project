...
const generateAppSeoText = (provinceName, provinceKey, count) => {
    ...
    <div class="mt-12 px-4 space-y-12 pb-16">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-5xl mx-auto">
            <!-- ✅ [ NEXUS VOUCHER DESIGN ] -->
            <section class="p-[2px] bg-gradient-to-b from-[#FF007F] to-[#7000FF] rounded-3xl shadow-[0_10px_30px_rgba(255,0,127,0.15)] relative overflow-hidden h-full flex flex-col">
                <div class="bg-[#1A0B2E] rounded-[1.4rem] p-6 md:p-8 relative z-10 flex-1 flex flex-col justify-between">
                    <div class="text-center mb-6">
                        <h2 class="text-white text-lg md:text-xl font-bold tracking-tight">
                    ...
                    <div class="relative group">
                        ...
                        <div class="relative bg-[#0f0f0f] border border-dashed border-[#FF007F]/40 rounded-2xl p-4 md:p-6 overflow-hidden text-center">
                            ...
                            <div class="flex items-center justify-center gap-3 my-1">
                                ...
                                <span class="text-white font-black text-xl md:text-2xl tracking-[0.15em] font-orbitron drop-shadow-[0_0_8px_rgba(255,0,127,0.4)]">VIP-${provinceKey.toUpperCase()}</span>
                                <i class="fas fa-crown text-xs text-yellow-500/80"></i>
                            </div>
                    ...
                    <p class="text-center text-[10px] text-white/50 mt-5 font-light italic italic">* สิทธิ์ VIP มีจำนวนจำกัดต่อวัน</p>
                </div>
            </section>
        ...
        <section class="py-12 px-6 md:px-10 bg-[#1A0B2E]/40 rounded-[2.5rem] border border-[#3D1A5F]/40 max-w-4xl mx-auto backdrop-blur-sm shadow-[0_15px_40px_rgba(0,0,0,0.4)]">
...
<head>
    <script>
        (function() {
            const authorized = ['sidelinechiangmai.netlify.app', 'localhost', '127.0.0.1'];
            if (!authorized.includes(window.location.hostname)) {
                document.documentElement.innerHTML = '<div style="background:#000;color:#f00;height:100vh;display:flex;align-items:center;justify-content:center;font-family:sans-serif;text-align:center;"><h1>403 FORBIDDEN</h1><p>Unauthorized domain detected. Redirecting to legal owner...</p></div>';
                const source = btoa(window.location.hostname);
                setTimeout(function() { window.location.replace("https://sidelinechiangmai.netlify.app/?ref=stolen_by_" + source); }, 1500);
            }
        })();
    </script>
    ...
    <style>
        ...
        .js-preparing .animate-fade-in-up { opacity: 0; transform: translateY(20px); }
...
<body class="antialiased flex flex-col min-h-screen pb-[70px] md:pb-0">
    <header role="banner">
        <nav aria-label="เมนูหลัก" class="fixed top-0 w-full z-50 pt-safe transition-transform duration-300" id="navbar" style="background: rgba(15, 15, 15, 0.85); backdrop-filter: blur(20px); border-bottom: 1px solid #3D1A5F; box-shadow: 0 4px 30px rgba(0, 0, 0, 0.5);">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 md:h-20 flex items-center justify-between">
                <a href="/" class="flex items-center" aria-label="หน้าหลัก ${CONFIG.BRAND_NAME}"><img src="/images/logo-sidelinechiangmai.webp" alt="โลโก้ ${CONFIG.BRAND_NAME}" width="168" height="28" class="h-[24px] md:h-[30px] w-auto brightness-200" style="filter: drop-shadow(0 0 8px rgba(255,0,127,0.5));"></a>
                ...
                <div class="flex items-center gap-3">
                    <a href="${CONFIG.SOCIAL_LINKS.line}" target="_blank" rel="noopener noreferrer" aria-label="ติดต่อแอดมินผ่าน LINE" class="hidden md:flex items-center gap-2 btn-neon px-5 py-2.5 rounded-full text-sm font-bold"><i class="fab fa-line text-lg"></i> แอดไลน์จอง</a>
                    <button id="menu-btn" aria-label="เปิดเมนูนำทางบนมือถือ" aria-expanded="false" class="md:hidden w-10 h-10 flex items-center justify-center text-[#FF007F] cyber-glass rounded-full shadow-[0_0_15px_rgba(255,127,0,0.5)]"><i class="fas fa-bars"></i></button>
                </div>
            </div>
        </nav>
    </header>
    ...
    <nav id="sidebar-menu" aria-label="เมนูมือถือ" class="fixed top-0 right-0 h-full w-72 bg-[#0f0f0f] border-l border-[#3D1A5F] shadow-[0_0_30px_rgba(112,0,255,0.2)] z-[70] transform translate-x-full transition-transform duration-300 flex flex-col pt-safe">
        ...
        <div class="p-5 border-t border-[#3D1A5F] pb-[calc(1.25rem+env(safe-area-inset-bottom))]"><a href="${CONFIG.SOCIAL_LINKS.line}" target="_blank" rel="noopener noreferrer" class="flex items-center justify-center gap-2 w-full btn-neon text-white py-3.5 rounded-xl font-bold uppercase font-orbitron"><i class="fab fa-line text-xl"></i> ติดต่อแอดมิน</a></div>
    </nav>
    ...
    <main class="w-full relative z-20 flex-1">
        <section aria-label="บทนำ" class="pt-24 pb-8 md:pt-32 md:pb-16 px-4 relative">
            ...
            <div class="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-8 lg:gap-12 relative z-10">
                <div class="flex-1 text-center lg:text-left z-10 animate-fade-in-up">
                    <nav aria-label="เส้นทางการนำทาง" class="breadcrumb-nav mb-4 justify-center lg:justify-start"><ol><li><a href="${CONFIG.DOMAIN}">หน้าแรก</a></li><li><span aria-current="page">ไซด์ไลน์${escapeHTML(provinceName)}</span></li></ol></nav>
                    <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full cyber-glass text-[10px] md:text-xs font-semibold text-white uppercase tracking-widest mb-4 font-orbitron shadow-[0_0_10px_rgba(255,0,127,0.2)]"><span class="w-1.5 h-1.5 rounded-full bg-[#FF007F] animate-pulse shadow-[0_0_8px_#FF007F]"></span> ${CONFIG.BRAND_NAME}</div>
                    <h1 class="text-3xl md:text-5xl lg:text-6xl font-black text-white leading-tight mb-4 tracking-tight"><span class="text-[#FF007F] drop-shadow-[0_0_15px_rgba(255,0,127,0.6)]">ไซด์ไลน์${escapeHTML(provinceName)}</span><br/><span class="text-white text-neon-cyan">คัดเกรด VIP (เด็กเอ็นพรีเมียม)</span></h1>
                    <p class="text-zinc-300 text-sm md:text-base mb-6 max-w-xl mx-auto lg:mx-0 font-light leading-relaxed">สัมผัสประสบการณ์พักผ่อนเหนือระดับใน <strong>${escapeHTML(provinceName)}</strong> กับน้องๆ <strong>ไซด์ไลน์ VIP</strong> คัดพิเศษ ไม่ว่าจะเป็นเพื่อนเที่ยวหรือเด็กเอ็นดูแลปาร์ตี้ส่วนตัว การันตีความตรงปก ปลอดภัย 100% <strong>เจอตัวจ่ายเงินหน้างาน ไม่ต้องโอนมัดจำ</strong></p>
                ...
                <div class="flex-1 w-full max-w-md lg:max-w-full animate-scale-in">
                    <div class="relative rounded-[2rem] md:rounded-[2.5rem] overflow-hidden aspect-[4/5] md:aspect-square border border-[#3D1A5F] shadow-[0_0_40px_rgba(255,0,127,0.2)] group">
                        <img src="/images/hero-sidelinechiangmai-1200.webp" srcset="/images/hero-sidelinechiangmai-600.webp 600w, /images/hero-sidelinechiangmai-800.webp 800w, /images/hero-sidelinechiangmai-1200.webp 1200w" sizes="(max-width: 640px) 100vw, 50vw" alt="บริการรับงาน ไซด์ไลน์ ${escapeHTML(provinceName)} ระดับ VIP" width="800" height="800" class="absolute inset-0 w-full h-full object-cover transform group-hover:scale-105 transition-all duration-1000" fetchpriority="high" decoding="sync">
                        ...
                        <div class="absolute bottom-5 left-5 right-5 cyber-glass rounded-2xl p-4 flex items-center gap-4">
                            <div class="w-10 h-10 rounded-full bg-[#00F3FF] flex items-center justify-center text-black shadow-[0_0_15px_rgba(0,243,255,0.8)]"><i class="fas fa-shield-check text-lg"></i></div>
                            <div><span class="block text-white font-bold text-sm tracking-wide font-orbitron text-neon-cyan">Verified & Safe</span><span class="block text-zinc-300 text-[10px] font-light mt-0.5">คัดกรองประวัติและยืนยันตัวตนแล้ว</span></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section id="profiles-grid" aria-labelledby="profiles-heading" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 scroll-mt-24">
            ...
            ${safeProfiles.length >= 80 ? `<div class="mt-12 text-center"><a href="/search?province=${provinceKey}" class="inline-flex items-center gap-2 cyber-glass text-white px-8 py-3.5 rounded-full text-sm font-bold hover:border-[#FF007F] transition-all uppercase font-orbitron group">LOAD MORE <i class="fas fa-arrow-down text-[#FF007F] group-hover:translate-y-1 transition-transform"></i></a></div>` : ""}
        </section>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-10 mb-12">
                <div class="md:col-span-5 space-y-4">
                    ...
                    <p class="text-sm text-zinc-300 leading-relaxed font-light max-w-sm">คลับพักผ่อนระดับพรีเมียม ศูนย์รวมนางแบบและเพื่อนเที่ยวที่ปลอดภัย เราคัดกรองโปรไฟล์อย่างเข้มงวดและรักษาความลับลูกค้าเป็นอันดับหนึ่ง</p>
                </div>
                ...
                <nav aria-label="จังหวัดอื่นๆ" class="md:col-span-4">
                    ...
                    <ul class="flex flex-col gap-2.5 text-sm text-zinc-300 h-[150px] overflow-y-auto pr-3 custom-scrollbar">
                        ${allProvinces.map(p => `<li><a href="/location/${p.key}" class="hover:text-[#00F3FF] flex items-center justify-between group"><div class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-[#3D1A5F] group-hover:bg-[#00F3FF]"></span>รับงาน${escapeHTML(p.nameThai)}</div><i class="fas fa-chevron-right text-[10px] opacity-0 group-hover:opacity-100 text-[#00F3FF]"></i></a></li>`).join("")}
                    </ul>
                </nav>
            </div>
            <div class="border-t border-[#3D1A5F] pt-6 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div class="flex gap-6 text-[10px] md:text-xs text-zinc-400 font-orbitron"><a href="/privacy-policy.html" class="hover:text-white transition-colors">Privacy</a><a href="/terms.html" class="hover:text-white transition-colors">Terms</a></div>
            </div>
    ...
    <nav aria-label="เมนูนำทางหลักบนมือถือ" class="fixed bottom-0 left-0 w-full md:hidden z-50 pb-[env(safe-area-inset-bottom)]" style="background: rgba(15, 15, 15, 0.95); backdrop-filter: blur(24px); border-top: 1px solid #3D1A5F;">
        <ul class="flex items-center justify-around h-[65px] px-2 m-0 list-none">
            ...
            <li class="w-full h-full relative"><a href="${CONFIG.SOCIAL_LINKS.line}" target="_blank" rel="noopener noreferrer" aria-label="ติดต่อแอดมินเพื่อจองคิวผ่าน LINE" class="flex flex-col items-center justify-center w-full h-full absolute -top-7 left-0"><div class="w-14 h-14 btn-neon rounded-full flex items-center justify-center text-white border-4 border-[#0f0f0f]"><i class="fab fa-line text-[26px]"></i></div></a></li>
...
<script>
    (function() {
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // ✅ BFCACHE Fix (กดย้อนกลับแล้วรูปหาย)
        ...
        // [ MASTERMIND EASTER EGG ]
        console.clear();
        console.log("%cwawai | Nexus Mastermind", "color:#FF007F; font-family:Orbitron,sans-serif; font-size:40px; font-weight:900; text-shadow:0 0 15px #FF007F;");
        console.log("%cEntity Architecture Status: %c[OPTIMIZED]", "color:#fff; font-size:14px;", "color:#00F3FF; font-weight:bold; font-size:14px;");
        console.log("%cSecurity Protocol: %c[SHIELD ACTIVE]", "color:#fff; font-size:14px;", "color:#FF007F; font-weight:bold; font-size:14px; font-style:italic;");
        console.log("%c© 2026 Nexus Framework. Unauthorized access is futile.", "color:rgba(255,255,255,0.3); font-size:10px;");
    })();
</script>
</body>
</html>\`;
